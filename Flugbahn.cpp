...
/* second order */
    ...
    //dr[2] = (-1.0)*G*M/pow(sqrt((pow(r[0],2)+pow(r[1],2))),3)*r[0]-(1/2)*(c_w)*A*rho*pow(r[2], 2);				//zweite Ableitungen (r und phi)
    //dr[3] = (-1.0)*G*M/pow(sqrt((pow(r[0],2)+pow(r[1],2))),3)*r[1]-(1/2)*(c_w)*A*rho*pow(r[3], 2);

    dr[2] = (-1.0)*G*M/pow(sqrt((pow(r[0],2)+pow(r[1],2))),3)*r[0]-(1/2)*(c_w)*A*(rho0*exp(-(pow(r[0],2)+pow(r[1],2))/H0))*pow(r[2], 2);				//zweite Ableitungen (r und phi)
